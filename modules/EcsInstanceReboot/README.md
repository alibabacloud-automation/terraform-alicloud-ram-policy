...
```hcl
module "example" {
  source = "terraform-alicloud-modules/ram-policy/alicloud//modules/EcsInstanceReboot"
...
| <a name="input_description"></a> [description](#input\_description) | The description of the policy | `string` | `"Allow to list all ECS instances and reboot specified instances."` | no |
| <a name="input_ecs_instance_ids"></a> [ecs\_instance\_ids](#input\_ecs\_instance\_ids) | The ID of ECS instances, default is all instances | `list(string)` | <pre>[<br/>  "*"<br/>]</pre> | no |
| <a name="input_policy_name"></a> [policy\_name](#input\_policy\_name) | The name of the policy | `string` | `"EcsInstanceReboot"` | no |
