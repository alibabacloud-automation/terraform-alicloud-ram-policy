...
```hcl
module "example" {
  source = "terraform-alicloud-modules/ram-policy/alicloud//modules/DatabaseAdministrator"
}
```
...
| <a name="input_description"></a> [description](#input\_description) | The description of the policy | `string` | `"Database administrator, who is responsible for enterprise databases operation, can open and create database services, and has all permissions for database services."` | no |
| <a name="input_policy_name"></a> [policy\_name](#input\_policy\_name) | The name of the policy | `string` | `"DatabaseAdministrator"` | no |
