# EcsInstanceRunCommand

## Usage

Create a policy that allow to list all ECS instances, run and stop commands on instances.

```hcl
module "example" {
  source = "terraform-alicloud-modules/ram-policy/alicloud//modules/EcsInstanceRunCommand"
}
```
...
| <a name="input_description"></a> [description](#input\_description) | The description of the policy | `string` | `"Allow to list all ECS instances, run and stop commands on instances."` | no |
| <a name="input_policy_name"></a> [policy\_name](#input\_policy\_name) | The name of the policy | `string` | `"EcsInstanceRunCommand"` | no |
