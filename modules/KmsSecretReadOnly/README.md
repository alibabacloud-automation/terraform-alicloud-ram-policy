...
Create a policy that allow to list and view all KMS secrets.

```hcl
module "example" {
  source = "terraform-alicloud-modules/ram-policy/alicloud//modules/KmsSecretReadOnly"
}
```
...
| <a name="input_description"></a> [description](#input\_description) | The description of the policy | `string` | `"Allow to list and view all KMS secrets."` | no |
| <a name="input_policy_name"></a> [policy\_name](#input\_policy\_name) | The name of the policy | `string` | `"KmsSecretReadOnly"` | no |
