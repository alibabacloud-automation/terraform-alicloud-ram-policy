# RedisDbInstanceAccount

## Usage

Create a policy that allow to list all Redis instances, create account and reset password and modify securify IPs.

```hcl
module "example" {
  source = "terraform-alicloud-modules/ram-policy/alicloud//modules/RedisDbInstanceAccount"
}
```
...
| <a name="input_description"></a> [description](#input\_description) | The description of the policy | `string` | `"Allow to list all Redis instances, create account and reset password and modify securify IPs."` | no |
| <a name="input_policy_name"></a> [policy\_name](#input\_policy\_name) | The name of the policy | `string` | `"RedisDbInstanceAccount"` | no |
