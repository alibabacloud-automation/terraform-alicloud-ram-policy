...
Create a policy for auditing administrator, who can access Cloud Config, ActionTrail, SLS and describe all cloud resources.

```hcl
module "example" {
  source = "terraform-alicloud-modules/ram-policy/alicloud//modules/AuditAdministrator"
}
```
...
| <a name="input_description"></a> [description](#input\_description) | The description of the policy | `string` | `"Auditing administrator, who can access Cloud Config, ActionTrail, SLS and describe all cloud resources."` | no |
| <a name="input_policy_name"></a> [policy\_name](#input\_policy\_name) | The name of the policy | `string` | `"AuditAdministrator"` | no |
